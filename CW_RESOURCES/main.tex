...
Markov chains are widely used in quantitative modeling to describe systems that evolve between discrete states over time. They provide a structured way to capture how current conditions influence future outcomes based on historical patterns. Together with \textbf{Vishesh Mongia - CID X}, we model a time series as a sequence of latent states, with transitions between states represented probabilistically on a directed graph. This framework enables the analysis of temporal dynamics such as persistence and mean reversion, which are commonly observed in real-world data.
...
A concern was that some of the transition probabilities in figure 22 were rather noisy. For example, the most probable next move from a large upswing was a large downswing at 22.2\% of the time! This seemed quite counter-intuitive, and prompted a better formulation of node states.\\
...
In practice, we probably wouldn't use the HMM directly as a trading strategy. Instead one could feed these discovered regime features ( duration, type etc.) into a larger model (XGBoost, LSTM, etc.) alongside other standard price/volume features. Another feature from transformer-based models is the Mixture of Experts (MoE) layer, which could be used to route to different sub-networks based on the inferred regime, allowing for regime-specific modelling of returns.\\
